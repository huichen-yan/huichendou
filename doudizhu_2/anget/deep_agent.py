...
from huichen_dou.doudizhu_2.anget import deep_pre


def landlord_choose_action(card0,card1,card0_opp,former_card,cc0,playable_cards,playing_record):
    ...
    farmer_played_cards_batch = dou_2.get_batch(cc0, len(playable_cards)).batch()
    farmer_num_cards_left_batch = dou_2.c_card(card1, 17, len(playable_cards))._get_one_hot_()
    ...
    x_batch = np.hstack((
        ...
        dou_2.get_batch("", len(playable_cards)).batch(),
        # farmer_played_cards_batch,
        farmer_num_cards_left_batch,
        dou_2.c_card("", 17, len(playable_cards))._get_one_hot_(),
        # farmer_num_cards_left_batch,
        bomb_num_batch,
        my_action_batch
    ))
...
def farmer_choose_action(card1,card1_opp,cc1,former_card,card0,playable_cards,playing_record):
    ...
    landlord_played_cards_batch = dou_2.get_batch(cc1, len(playable_cards)).batch()
    teammate_played_cards_batch = dou_2.get_batch("", len(playable_cards)).batch()
    last_action_batch = dou_2.get_batch(former_card, len(playable_cards)).batch()
    last_landlord_action_batch = last_action_batch
    last_teammate_action_batch = teammate_played_cards_batch
    landlord_num_cards_left_batch = dou_2.c_card(card0, 20, len(playable_cards))._get_one_hot_()
    teammate_num_cards_left_batch = dou_2.c_card([], 17, len(playable_cards))._get_one_hot_()
